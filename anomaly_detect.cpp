...
// Function to create plotter files
void create_plot_files(string input_file) {
    ...
    ofstream p_plot("p_plot.txt");  // For pitch drift rates
    ofstream r_plot("r_plot.txt");  // For roll drift rates
    ofstream y_plot("y_plot.txt");  // For yaw drift rates
    
    if (!p_plot.is_open() || !r_plot.is_open() || !y_plot.is_open()) {
    ...
    while (getline(infile, line)) {
        ...
        string fid;
        double pitch_drift, roll_drift, yaw_drift;
        
        // Parse the line, assuming the columns are in the same format as provided
        ss >> hour >> minute >> second >> fid >> pitch_drift >> roll_drift >> yaw_drift;
        
        // Write to the respective plot files
        p_plot << hour << " " << pitch_drift << endl;
        r_plot << hour << " " << roll_drift << endl;
        y_plot << hour << " " << yaw_drift << endl;
    }
    ...
    cout << "Plot files created successfully!" << endl;
}
